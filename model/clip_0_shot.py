...
model_name = 'clip'
save_file_name = '../result_other_clips/'+model_name+'rn50/'
...
def albef_zero_shot_eval():
    ...
    for language in languages:
        mrr = [[] for _ in range(8)]
        hit = [[] for _ in range(8)]
            ...
            with tqdm(data_loader, dynamic_ncols=True) as tqdmDataLoader:
                for batch in tqdmDataLoader:
                    ...
                    for image in raw_images:
                        ...
                        image_feature = model.extract_features(sample_a)
                        image_features.append(image_feature.image_embeds_proj)

                    image_features = torch.cat(image_features)
                    image_feature = image_features.squeeze()

                    text_samples = [sample_a, sample_ab, sample_prompt_ab, sample_wn_c, sample_wn_t, sample_wn_prompt_t, sample_gpt_ab, sample_prompt_gpt_ab]
                    # text_samples = []
                    test_method = 0
                    for sample in text_samples:
                        rank_val_a, rank_label_a, image_idx_a, probs_a, type_a = get_measure(sample, model, image_feature,image_list,img_file_type,label)
...
def get_measure(text_feature, model, image_feature,image_list,img_file_type,label):
    text_feature_a = model.extract_features(text_feature)
...
def get_similarity(text_feature, image_feature, image_list, file_type_list, label):
    ...
    for ind in range(10):
        logits_per_image = (image_feature[ind].unsqueeze(0) @ text_feature.t())
